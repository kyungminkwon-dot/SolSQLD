...
작업이 완료되면 반드시 아래 절차를 따른다:

1. 작업 결과를 사용자에게 요약 보고
2. **"슬랙 알림을 받으시겠습니까?"** 라고 질문
3. 사용자가 수락하면 `~/.claude/hooks/notify-slack.sh` 를 호출하여 알림 전송
4. 사용자가 거절하면 알림을 보내지 않음

```bash
# 호출 형식
~/.claude/hooks/notify-slack.sh "<작업명>" "<작업 요약>" "<success|fail>"
```
