  ...
  return (
    <div className="min-h-screen bg-slate-50">
      <div className="max-w-4xl mx-auto px-4 py-10">
        ...
        <p className="text-slate-500 text-sm mb-8">실제 SQLD 시험 형식의 모의고사로 실력을 점검하세요.</p>
                ...
                <div>
                  ...
                  <div className="flex items-center gap-3 text-xs text-slate-500">
                    <span className="flex items-center gap-1">
                      <FileText className="w-3.5 h-3.5" />{exam.problemCount}문항
                    </span>
                    <span className="flex items-center gap-1">
                      <Clock className="w-3.5 h-3.5" />{exam.timeLimit}분
                    </span>
                    <span className={`px-2 py-0.5 rounded-full font-semibold ${DIFFICULTY_COLOR[exam.avgDifficulty]}`}>
              ...
              <div className="flex items-center gap-4">
                {exam.isSolved && exam.bestScore !== null && (
                  <div className="text-right">
                    ...
                    <p className={`font-bold text-lg ${exam.bestScore >= 60 ? 'text-emerald-600' : 'text-red-500'}`}>
                      {exam.bestScore}점
                    </p>
                  </div>
                )}
